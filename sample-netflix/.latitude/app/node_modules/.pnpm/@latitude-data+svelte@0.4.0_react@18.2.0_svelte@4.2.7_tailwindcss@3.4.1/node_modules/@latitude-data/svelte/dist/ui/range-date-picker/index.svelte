<script context="module">import { RichDate, RELATIVE_RANGES } from "@latitude-data/custom_types";
</script>
...
import { Button, RangeCalendar, Popover, Select, ToggleGroup } from "../..";
...
const selectorItems = Object.keys(RELATIVE_RANGES).map((key) => {
...
const relativeDateLabel = (value2) => {
  return Object.entries(RELATIVE_RANGES).find(([_, v]) => v.start === value2.start.value && v.end === value2.end.value)?.[0] ?? "";
};
$:
  selectorValue = {
    label: isRelative ? relativeDateLabel($val) : void 0,
    value: isRelative ? relativeDateLabel($val) : void 0
  };
